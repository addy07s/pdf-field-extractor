...
def test_smoke_values_happy_path(field_configs) -> None:
    ...
    assert result.fields["gstin"].status == FieldStatus.OK
    assert result.fields["pan"].status == FieldStatus.OK
    assert result.fields["invoice_date"].value == {
        "original": "23/04/2026",
        "normalized": "2026-04-23",
    }
    assert result.fields["taxable_amount"].value == 4380.0
    assert result.fields["gst_amount"].value == 788.4
    assert result.fields["total_amount"].value == 5168.4
    assert result.fields["taxable_amount"].status == FieldStatus.OK
    assert result.fields["gst_amount"].status == FieldStatus.OK
    assert result.fields["total_amount"].status == FieldStatus.OK
...
def test_smoke_corrupted_gstin_transposition_fails_checksum(field_configs) -> None:
    ...
    assert result.fields["gstin"].status == FieldStatus.FAILED_VALIDATION
    assert "GSTIN checksum failed" in (result.fields["gstin"].reason or "")


def test_smoke_broken_reconciliation_flags_all_amounts(field_configs) -> None:
    """Smoke taxable + GST with a wrong total must flag all three amount fields."""
    raw = dict(SMOKE_RAW)
    raw["taxable_amount"] = "4380.00"
    raw["gst_amount"] = "788.40"
    raw["total_amount"] = "9999.99"
    ...
    for key in ("taxable_amount", "gst_amount", "total_amount"):
...
def test_smoke_gstin_pan_mismatch_flags_both_fields(field_configs) -> None:
    """Smoke GSTIN with a wrong PAN must flag both fields on cross-check."""
    raw = dict(SMOKE_RAW)
    raw["gstin"] = "24ABMFA4190N1Z2"
    raw["pan"] = "ZZZZZ9999Z"
    ...
    assert "GSTIN/PAN mismatch" in (result.fields["gstin"].reason or "")
...
def test_none_field_is_not_found(field_configs) -> None:
    ...
    assert result.fields["gstin"].status == FieldStatus.NOT_FOUND
    assert result.fields["gstin"].reason == "field not extracted"
